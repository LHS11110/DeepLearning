...
5. $u \times u = 0$
## 직선의 방정식
- 3차원 공간에서 위치벡터가 $a$인 점 $A$를 지나며 방향벡터가 $v$인 직선상의 임의의 점 $X$의 위치벡터 $x$는 $x = a + kv$를 만족한다. 단 $k$는 임의의 실수
## 평면의 방정식
- 법선벡터: 벡터 $x$에 수직하는 벡터 $n$
- 3차원 공간에서 위치벡터가 $a$인 점 $A$를 지나며 법선벡터가 $v$인 평면상의 임의의 점 $X$의 위치벡터 $x$는 $(x-a) \cdot v = 0$